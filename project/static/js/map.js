...
$("#input-location").on("keyup", function(e) {
  ...
  if ($("#input-location").val() !== "") {
    ...
    $.ajax({
      method: "GET",
      url: `https://maps.googleapis.com/maps/api/geocode/json?address=${selectedLocation}&key=AIzaSyDnepJu6LjYHFcOyikx01Fn9KTQjc62KYs`
    }).then(function(data) {
      if (data["status"] === "OK") {
        locationData = data;
        $("#search-list").empty();
        data["results"].forEach(function(val, idx) {
          $("#search-list").append(
            `<li id=${idx}>${val["formatted_address"]}</li>`
          );
        });
      }
    });
...
function getWeatherData() {
  ...
  $.ajax({
    ...
    $(
      "#weather-msg"
    ).html(`<div class="alert alert-info" role="alert">The temperature is currently <strong>${temp}&degF</strong> 
...
function initClock() {
  ...
  $("#clock").html(
    `${hours}:${minutes}<span id='seconds'>${seconds}</span> <span id='ampm'>${amPm}</span>`
  );
